...
Napi::Object QOpenGLShaderProgramWrap::init(Napi::Env env, Napi::Object exports) {
  ...
  Napi::Function func = DefineClass(
      ...
      InstanceMethod("disableAttributeArray", &QOpenGLShaderProgramWrap::disableAttributeArray),
      InstanceMethod("enableAttributeArray", &QOpenGLShaderProgramWrap::enableAttributeArray),
      ...
      InstanceMethod("setAttributeBuffer",  &QOpenGLShaderProgramWrap::setAttributeBuffer),
      InstanceMethod("setUniformMatrix4fv", &QOpenGLShaderProgramWrap::setUniformMatrix4fv),
      InstanceMethod("setUniformValue1i",  &QOpenGLShaderProgramWrap::setUniformValue1i),
...
Napi::Value QOpenGLShaderProgramWrap::setUniformValue1i(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);
...
Napi::Value QOpenGLShaderProgramWrap::disableAttributeArray(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);
...
Napi::Value QOpenGLShaderProgramWrap::enableAttributeArray(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);
...
Napi::Value QOpenGLShaderProgramWrap::setUniformMatrix4fv(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);

  GLint location = info[0].As<Napi::Number>().Int32Value();
  auto arrayBuffer = info[1].As<Napi::ArrayBuffer>(); // Should be full of GLfloat
  QMatrix4x4 matrix(static_cast<GLfloat*>(arrayBuffer.Data()));
  this->instance->setUniformValue(location, matrix);
  return env.Null();
}

Napi::Value QOpenGLShaderProgramWrap::setAttributeBuffer(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);
...
Napi::Value QOpenGLShaderProgramWrap::programId(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  Napi::HandleScope scope(env);
